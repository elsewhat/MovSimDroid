...
public class MovSimTrafficView extends ViewBase implements
	...
	private final DashPathEffect roadLineDashPathEffect = new DashPathEffect(
			new float[] { 10, 20 }, 1);
	...
	public MovSimTrafficView(Context context, Simulator simulator,
	...
	public void resetGraphicproperties() {
		Properties properties = ViewProperties.loadProperties(
	...
	/**
	 * Callback to allow the application to make any further required drawing
	 * after the vehicles have been moved.
	 */
	protected void drawAfterVehiclesMoved(Canvas canvas, double simulationTime,
			long iterationCount) {
	}

	/**
	 * <p>
	 * Draws the foreground: everything that moves each timestep. For the
	 * traffic simulation that means draw all the vehicles:<br />
	 * For each roadSection, draw all the vehicles in the roadSection,
	 * positioning them using the roadMapping for that roadSection.
	 * </p>
	 * 
	 * <p>
	 * This method is synchronized with the
	 * <code>SimulationRunnable.run()</code> method, so that vehicles are not
	 * updated, added or removed while they are being drawn.
	 * </p>
	 * <p>
	 * tm The abstract method paintAfterVehiclesMoved is called after the
	 * vehicles have been moved, to allow any further required drawing on the
	 * canvas.
	 * </p>
	 * 
	 * @param canvas
		...
		synchronized (simulationRunnable.dataLock) {
			...
			for (final RoadSegment roadSegment : roadNetwork) {
				...
				if (androidVersion < 12) {
					DrawRoadMapping.clipPath(canvas, clipPath, roadMapping); // TODO
																				// clipPath
																				// not
																				// supported
																				// on
			...
			totalAnimationTime += System.currentTimeMillis()
					- timeBeforePaint_ms;

			drawAfterVehiclesMoved(canvas, simulationRunnable.simulationTime(),
					simulationRunnable.iterationCount());

		}
	}

	private void drawVehicle(Canvas canvas, double simulationTime,
			RoadMapping roadMapping, Vehicle vehicle) {
		...
		vehiclePaint.setColor(vehicleColor(vehicle,
				simulationRunnable.simulationTime()));
	...
	/**
	 * Draws the background: everything that does not move each timestep. The
	 * background consists of the road segments and the sources and sinks, if
	 * they are visible.
	 * 
	 * @param canvas
	 */
	...
	private void drawRoadSegments(Canvas canvas) {
		...
		for (final RoadSegment roadSegment : roadNetwork) {
			...
			drawRoadSegmentLines(canvas, roadMapping); // in one step (parallel
														// or sequential
														// update)?!
		}
	}
	...
	private void drawRoadSegment(Canvas canvas, RoadMapping roadMapping) {
		...
		final double lateralOffset = 0.5 * roadMapping.trafficLaneMin()
				* roadMapping.laneWidth();
		final Path path = DrawRoadMapping.drawRoadMapping(roadPath,
				roadMapping, lateralOffset);
		if (path == null) {
			// default drawing splits the road into line sections and draws
			// those
			final double roadLength = roadMapping.roadLength();
			final double sectionLength = 20; // draw the road in sections 20
												// meters long
			double roadPos = 0.0;
	...
	private void drawTrafficLightsOnRoad(Canvas canvas, RoadSegment roadSegment) {
		...
		final int offset = (int) ((roadMapping.laneCount() / 2.0 + 1.5) * roadMapping
				.laneWidth());
		...
		for (final TrafficLight trafficLight : roadSegment.trafficLights()) {
			...
			final RoadMapping.PosTheta posTheta = roadMapping.map(
					trafficLight.position(), offset);
			Rect rect = new Rect((int) posTheta.x + offset - radius,
					(int) posTheta.y + offset - radius, (int) posTheta.x
							+ offset + radius, (int) posTheta.y + offset
							+ radius);
			// trafficLightRegion = new Region(new Rect((int) posTheta.x +
			// offset - radius-(int)trafficLight.position(), (int) posTheta.y +
			// offset - radius, (int) posTheta.x + offset +
			// radius-(int)trafficLight.position(), (int) posTheta.y + offset +
			// radius));
			canvas.drawRect(rect, paint);
			final TrafficLightStatus status = trafficLight.status();
			setTrafficlightPaint(status);
			canvas.drawCircle((int) posTheta.x + offset, (int) posTheta.y
					+ offset, radius, paint);
		}
	}
	...
	private void drawSpeedLimitsOnRoad(Canvas canvas, RoadSegment roadSegment) {
		...
		final RoadMapping roadMapping = roadSegment.roadMapping();
		assert roadMapping != null;
		final double offset = -(roadMapping.laneCount() / 2.0 + 1.5)
				* roadMapping.laneWidth();
		...
		for (final SpeedLimit speedLimit : roadSegment.speedLimits()) {
			...
			final RoadMapping.PosTheta posTheta = roadMapping.map(
					speedLimit.getPosition(), offset);

			final double speedLimitValueKmh = speedLimit.getSpeedLimitKmh();
			if (speedLimitValueKmh < 150) {
				paint.setColor(0xffee1111);
				canvas.drawCircle((int) posTheta.x + xOffset, (int) posTheta.y
						+ redRadius - offsetY, redRadius, paint);
				paint.setColor(0xffeeeeee);
				canvas.drawCircle((int) posTheta.x + xOffset, (int) posTheta.y
						+ redRadius - offsetY, whiteRadius, paint);

				final String text = String.valueOf((int) (speedLimit
						.getSpeedLimitKmh()));
	...
	private void drawRoadSectionIds(Canvas canvas) {
		for (final RoadSegment roadSegment : roadNetwork) {
			final RoadMapping roadMapping = roadSegment.roadMapping();
			assert roadMapping != null;
			// final int radius = (int) ((roadMapping.laneCount() + 2) *
			// roadMapping.laneWidth());
			...
			canvas.drawText("Info: " + roadSegment.userId(),
					(int) (posTheta.x + 16), (int) (posTheta.y + 16), paint);
		}
	}
	...
	private void drawSources(Canvas canvas) {
		...
		for (final RoadSegment roadSegment : roadNetwork) {
			final RoadMapping roadMapping = roadSegment.roadMapping();
			assert roadMapping != null;
			final int radius = (int) ((roadMapping.laneCount() + 2) * roadMapping
					.laneWidth());
			...
			if (trafficSource != null) {
				...
				canvas.drawCircle((int) posTheta.x, (int) posTheta.y, radius,
						paint);
				...
				inflowStringBuilder
						.append((int) (Units.INVS_TO_INVH * trafficSource
				...
				inflowStringBuilder.append(")");
				canvas.drawText(inflowStringBuilder.toString(),
						(int) (posTheta.x) + radius, (int) (posTheta.y)
								+ radius, paint);
			}
		}
	}
	...
	private void drawSinks(Canvas canvas) {
		...
		for (final RoadSegment roadSegment : roadNetwork) {
			...
			if (sink != null) {
				...
				canvas.drawCircle((int) posTheta.x, (int) posTheta.y, radius,
						paint);
				String outflowString = "outflow: "
						+ (int) (Units.INVS_TO_INVH * sink.measuredOutflow())
						+ " veh/h";
				// outflow text
				paint.setAntiAlias(true);
				paint.setTextSize(20);
				canvas.drawText(outflowString, (int) (posTheta.x) + radius,
						(int) (posTheta.y) + radius, paint);
			}
		}
	}

	/**
	 * Returns the color of the vehicle. The color may depend on the vehicle's
	 * properties, such as its velocity.
	 * 
	 * @param vehicle
	...
	protected int vehicleColor(Vehicle vehicle, double simulationTime) {
		...
		case VELOCITY_COLOR:
			if (vehicle.exitRoadSegmentId() != Vehicle.ROAD_SEGMENT_ID_NOT_SET) {
				...
				color = getColorAccordingToSpectrum(0,
						getVmaxForColorSpectrum(), v);
			}
			break;
	...
	public int getColorAccordingToSpectrum(double vmin, double vmax, double v) {
		...
		float vRelative = (vmax > vmin) ? (float) ((v - vmin) / (vmax - vmin))
				: 0;
	...
	private int[] hsv2rgb(float h, float s, float v) {
		...
		switch (i) {
		case 0:
			return new int[] { (int) (v * 256), (int) (t * 256),
					(int) (p * 256) };
		case 1:
			return new int[] { (int) (q * 256), (int) (v * 256),
					(int) (p * 256) };
		case 2:
			return new int[] { (int) (p * 256), (int) (v * 256),
					(int) (t * 256) };
		case 3:
			return new int[] { (int) (p * 256), (int) (q * 256),
					(int) (v * 256) };
		case 4:
			return new int[] { (int) (t * 256), (int) (p * 256),
					(int) (v * 256) };
		case 5:
			return new int[] { (int) (v * 256), (int) (p * 256),
					(int) (q * 256) };
		}
		return null;
	}
	...
	protected void initGraphicConfigFieldsFromProperties(Properties properties) {
		...
		setDrawSinks(Boolean.parseBoolean(properties.getProperty("drawSinks",
				"true")));
		setDrawSources(Boolean.parseBoolean(properties.getProperty(
				"drawSources", "true")));
		setDrawSlopes(Boolean.parseBoolean(properties.getProperty("drawSlopes",
				"true")));
		setDrawSpeedLimits(Boolean.parseBoolean(properties.getProperty(
				"drawSpeedLimits", "true")));

		backgroundColor = Color.parseColor("#"
				+ properties.getProperty("backgroundColor", "303030"));
		roadColor = Color.parseColor("#"
				+ properties.getProperty("roadColor", "808080"));
		roadEdgeColor = Color.parseColor("#"
				+ properties.getProperty("roadEdgeColor", "222222"));
		roadLineColor = Color.parseColor("#"
				+ properties.getProperty("roadLineColor", "DDDDDD"));
		sourceColor = Color.parseColor("#"
				+ properties.getProperty("sourceColor", "FFFFFF"));
		sinkColor = Color.parseColor("#"
				+ properties.getProperty("sinkColor", "000000"));
		setVehicleColorMode(VehicleColorMode.valueOf(properties.getProperty(
				"vehicleColorMode", "VELOCITY_COLOR")));
		setVmaxForColorSpectrum(Double.parseDouble(properties.getProperty(
				"vmaxForColorSpectrum", "140")));

		lineWidth = Float
				.parseFloat(properties.getProperty("lineWidth", "1.0"));
		lineLength = Float.parseFloat(properties.getProperty("lineLength",
				"5.0"));
		gapLength = Float.parseFloat(properties
				.getProperty("gapLength", "15.0"));
		gapLengthExit = Float.parseFloat(properties.getProperty(
				"gapLengthExit", "6.0"));

		scale = Float.parseFloat(properties.getProperty("initialScale",
				"0.707106781"));
		setSleepTime(Integer.parseInt(properties.getProperty(
	 ...
	 * 
	 * @param popupString
	 *            popup window format string for vehicle that leaves road
	 *            segment at a specific exit
	 * @param popupStringExitEndRoad
	 *            popup window format string for vehicle that leaves road
	 *            segment at end
	 */
	public void setMessageStrings(String popupString,
			String popupStringExitEndRoad) {
	...
	void setVelocityColors() {
		accelerationColors = new int[] { Color.WHITE, Color.RED, Color.BLACK,
				Color.GREEN };
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (touchMode == TOUCH_MODE_DRAG) {
				final float xOffsetNew = xOffsetSave
						+ (event.getX() - startDragX) / scale;
				final float yOffsetNew = yOffsetSave
						+ (event.getY() - startDragY) / scale;
				if (xOffsetNew != xOffset || yOffsetNew != yOffset) {
					// the user has dragged the view, so we need to redraw the
					// background bitmap
					xOffset = xOffsetNew;
					yOffset = yOffsetNew;
					setTransform();
				...
				if (pinchDistance > touchModeZoomHysteresis) {
					...
					setScale(newScale);
					// the user has zoomed the view, so we need to redraw the
					// background bitmap
					forceRepaintBackground();
				}
			}
